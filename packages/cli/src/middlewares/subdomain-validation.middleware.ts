...
export const subdomainValidationMiddleware = async (
	...
	try {
		...
		if (shouldSkip) {
			logger.debug(
				`[SubdomainValidation] Skipping validation for: ${req.url} (method: ${req.method}, upgrade: ${req.headers.upgrade})`,
			);
			return next();
		}
		...
		logger.info(
			`[SubdomainValidation] 🔍 Validating host: ${host} (forwarded: ${forwardedHost}, path: ${req.path})`,
		);
		...
		if (host.includes('localhost') || host.includes('127.0.0.1')) {
			...
			try {
				const dataSource =
					await VoyagerDataSourceFactory.getDataSourceForSubdomain(defaultSubdomain);
		...
		logger.info(
			`[SubdomainValidation] 🎯 Extracted subdomain: "${subdomain}" from host: "${host}"`,
		);
		...
		try {
			...
			logger.info(
				`[SubdomainValidation] ✅ DataSource ready - subdomain: "${subdomain}", database: "${dbName}"`,
			);
