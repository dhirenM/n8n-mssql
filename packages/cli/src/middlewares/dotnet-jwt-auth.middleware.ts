...
import { UserRepository, ProjectRepository, ProjectRelationRepository, RoleRepository } from '@n8n/db';
...
const AUDIENCE_SECRET = process.env.DOTNET_AUDIENCE_SECRET || 'fdbc6c9efcc14b2f-7299dae388174d8fb9c6ef8844';
...
export const dotnetJwtAuthMiddleware = async (
  ...
  try {
    ...
    if (req.headers.upgrade === 'websocket' || req.url.includes('/push')) {
    ...
    try {
      ...
      logger.warn('.NET JWT: Could not decode token for debugging', { error: (decodeError as Error).message });
    }
    ...
    logger.debug('.NET JWT validation config:', {
      ...
      isVirtuosoAI,
      secretSource: isVirtuosoAI ? 'AUDIENCE_SECRET (raw)' : 'DECODED_AUDIENCE_SECRET (base64url decoded)'
    });
    ...
    if (!dataSource) {
      logger.error('.NET JWT: No dataSource in request - subdomain validation middleware may not have run', {
        path: req.path,
        method: req.method,
...
async function createN8nUserFromDotNetJWT(payload: any, dataSource: DataSource): Promise<User> {
  ...
  const projectRelationRepo = dataSource.getRepository(Container.get(ProjectRelationRepository).target);
