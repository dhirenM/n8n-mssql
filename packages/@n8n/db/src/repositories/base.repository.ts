...
export abstract class BaseRepository<Entity extends ObjectLiteral> extends Repository<Entity> {
	...
	protected getContextManager(): EntityManager {
		try {
			...
			if (!contextDataSource.isInitialized) {
				console.warn(
					`[BaseRepository] ⚠️ DataSource for subdomain "${request.subdomain}" is not initialized, falling back to default`,
				);
				return this.manager;
			}
			...
			console.log(
				`[BaseRepository] ✅ Using tenant DB: ${dbName} (subdomain: ${request.subdomain})`,
			);
	...
	/**
	 * Override base TypeORM methods to use tenant-aware EntityManager
	 * This ensures that methods like findOneOrFail, find, count, etc.
	...
	override async find(options?: any) {
		const em = this.getContextManager();
		return await em.find(this.target, options);
	}

	override async findOne(options: any) {
		const em = this.getContextManager();
		return await em.findOne(this.target, options);
	}

	override async findBy(where: any) {
		const em = this.getContextManager();
		return await em.findBy(this.target, where);
	}

	override async findOneBy(where: any) {
		const em = this.getContextManager();
	...
	override async countBy(where: any) {
		const em = this.getContextManager();
	...
	override async delete(criteria: any) {
		const em = this.getContextManager();
		return await em.delete(this.target, criteria);
	}

	override async update(criteria: any, partialEntity: any) {
		const em = this.getContextManager();
		return await em.update(this.target, criteria, partialEntity);
	}

	override createQueryBuilder(alias?: string) {
		const em = this.getContextManager();
		return em.createQueryBuilder(this.target, alias || this.metadata.tableName);
	}

	/**
	 * Public accessor for getting the tenant-aware entity manager.
	 *
	 * **Use this in controllers/services when you need to run transactions.**
	 *
	 * ⚠️ DO NOT use `repository.manager` directly - it returns the default database!
	 *
	 * @example
	 ...
	 *
	 * // ✅ CORRECT - Uses tenant database:
	 * const manager = this.projectRepository.getManager();
	 * await manager.transaction(async (tx) => { ... });
	 *
	 * @returns EntityManager for the current tenant or default
	 */
